...
def _band_filter(arg):
    ...
    # narrowband signal
    narrowband_signal = lfilter(_narrowband_filters, 1,
                                np.concatenate([_bandpass_signal, np.zeros([_narrowband_filt_order // 2, 1])]), axis=0)
    ...
    for jj in range(0, number_of_events):
        values[jj, :] = [max(RMS_signal[event_start[jj]:event_end[jj], detection_channel[jj]] / moving_background[event_start[jj]:event_end[jj], detection_channel[jj]]),
                         ...
                         max(narrowband_signal[event_start[jj]:event_end[jj], detection_channel[jj]]),
                         min(narrowband_signal[event_start[jj]:event_end[jj], detection_channel[jj]]),
                         max(abs(np.diff(signal[event_start[jj]:event_end[jj], detection_channel[jj]])))
                         ]

    detected_events = np.concatenate([np.expand_dims(detection_channel, axis=1), fs * _freq_band * np.ones([number_of_events, 1]), np.expand_dims(event_start, axis=1), np.expand_dims(event_length, axis=1), values], axis=1)
...
def detect_hfo_nicolas(sig, fs, mp=1):
    ...
    minimum_separation_ripple = 0.02  # minimum separation between events [seconds]
    ...
    bandpass_filt = remez(numtaps=bandpass_filt_order,
                          bands=np.array([0, 65, 80, min(0.45 * fs, 500), min(0.45 * fs + 15, 515), 0.5 * fs]),
                          desired=np.array([0, 1, 0]),
                          grid_density=(bandpass_filt_order + 1) * 16,
                          weight=np.array([10, 1, 1]),
                          fs=fs)

    freq_band = np.array((80, 90, 105, 120, 140, 160, 185, 215, 250, 285, 330, 380, 435, 500)) / fs
    ...
    for ii in range(0, num_bands):
        narrowband_filters[ii:] = remez(numtaps=narrowband_filt_order,
                                        bands=np.array([0, freq_band[ii] - 7 / fs,
                                                        freq_band[ii] + 3 / fs,
                                                        freq_band[ii + 1] - 3 / fs,
    ...
    minimum_length = np.round(np.sqrt(np.sum(np.tile(np.arange(-narrowband_filt_order // 2, narrowband_filt_order // 2) ** 2, [num_bands, 1]) * narrowband_filters ** 2, 1) / np.sum(narrowband_filters ** 2, 1)) + 4 / freq_band)
    ...
    bandpass_signal = np.delete(bandpass_signal, np.arange(0, bandpass_filt_order // 2, dtype=int), axis=0)



    x = [[narrowband_filters[ii, :], freq_band[ii], bandpass_signal, narrowband_filt_order, window_length, threshold, num_samples, offset, minimum_length[ii], fs, sig] for ii in range(0, num_bands)]
    ...
    output = [tuple([int(oscillations[i,2]), int(oscillations[i,2]+oscillations[i,3]), oscillations[i,1]]) for i in np.arange(np.shape(oscillations)[0])]
