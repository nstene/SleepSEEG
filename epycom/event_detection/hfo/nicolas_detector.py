...
def _band_filter(arg):
    ...
    detected_events = np.concatenate([np.expand_dims(detection_channel, axis=1),
                                      fs * _freq_band * np.ones([number_of_events, 1]),
                                      np.expand_dims(event_start, axis=1), np.expand_dims(event_length, axis=1),
                                      values], axis=1)
...
def detect_hfo_nicolas(sig, fs=None, mp=1, threshold=3):
   ...
   threshold: float
    ...
    minimum_length = np.round(np.sqrt(np.sum(np.tile(np.arange(-narrowband_filt_order // 2,
